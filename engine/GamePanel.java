...
public class GamePanel extends JPanel implements Runnable, KeyListener {
    ...
    // ── Heal system ─────────────────────────────────────────────────────
    int healChargesLeft = 3; // 3 charges per level
    int healCooldown    = 0; // frames before next H-heal
    static final int HEAL_COOLDOWN_MAX = 180; // 3 seconds

    // ── Kunai (player ranged attack) ────────────────────────────────────
    int kunaiCooldown = 0;
    static final int KUNAI_COOLDOWN_MAX = 25; // frames (~0.4s)
    ...
    void update() {
        ...
        if (input.abilityJustPressed) {
            ...
            if (cost >= 0)
                healChargesLeft -= cost;
        }

        // ── H: Manual heal ────────────────────────────────────────────────
        if (input.healJustPressed && healChargesLeft > 0 && player.health < player.maxHealth) {
            int healed = Math.min(2, player.maxHealth - player.health);
        ...
        // ── K: Throw kunai ────────────────────────────────────────────────
        if (kunaiCooldown > 0) kunaiCooldown--;
        if (input.kunaiJustPressed && kunaiCooldown == 0) {
            kunaiCooldown = KUNAI_COOLDOWN_MAX;
            float kx    = player.facingRight ? player.x + Player.W + 2 : player.x - 6;
            float ky    = player.y + Player.H * 0.38f;
            float speed = 14f;
            float vx    = player.facingRight ? speed : -speed;
            bullets.add(new Bullet(kx, ky, vx, 0f, false, true));
            particles.spawn(kx, ky, ParticleSystem.Type.SPARK, player.facingRight);
        }
        ...
        while (bIter.hasNext()) {
            ...
            // ── Player kunai hits enemies ─────────────────────────────────
            if (b.isPlayerKunai) {
                boolean kunaiHit = false;
                if (state == GameState.PLAYING) {
                    for (Enemy e : currentLevel.enemies) {
                        if (!e.isDead() && b.getRect().intersects(e.getRect())) {
                            e.takeDamage(1);
                            camera.addTrauma(0.2f);
                            particles.spawn(e.x + Enemy.W / 2f, e.y + Enemy.H / 2f,
                                    ParticleSystem.Type.SPARK, b.velX > 0);
                            kunaiHit = true;
                            break;
                        }
                    }
                }
                if (!kunaiHit && state == GameState.BOSS_FIGHT && !boss.dead
                        && b.getRect().intersects(boss.getRect())) {
                    boss.takeDamage(1);
                ...
                continue; // kunai never hits player
            }

            // ── Enemy / boss bullets pass through invisible player ────────
            if (!player.isInvisible
    ...
    public void paintComponent(Graphics g) {
        ...
        if (phase.equals("NIGHT")) {
            skyTop = new Color(2, 2, 14);
            skyMid = new Color(6, 5, 28);
            skyBot = new Color(14, 10, 50);
        } else if (phase.equals("DUSK")) {
            skyTop = new Color(12, 3, 22);
            skyMid = new Color(80, 20, 40);
            skyBot = new Color(220, 80, 25);
        } else {
            skyTop = new Color(30, 90, 200);
            skyMid = new Color(90, 160, 240);
    ...
    private void drawCelestialBody(Graphics2D g2) {
        ...
        if (phase.equals("NIGHT")) {
            ...
            g2.fillOval(654, 36, 12, 10);
            g2.fillOval(660, 55, 9, 8);
            g2.fillOval(650, 65, 6, 6);
        } else if (phase.equals("DAY")) {
            g2.setColor(new Color(255, 230, 100, 30));
            g2.fillOval(110, 15, 100, 100);
            g2.setColor(new Color(255, 240, 120, 60));
            g2.fillOval(125, 28, 74, 74);
            g2.setPaint(new GradientPaint(135, 35, new Color(255, 255, 180), 162, 62, new Color(255, 200, 50)));
            g2.fillOval(140, 38, 55, 55);
        } else {
            g2.setColor(new Color(255, 100, 30, 80));
            g2.fillOval(90, HEIGHT - 80, 200, 120);
            g2.setColor(new Color(255, 140, 50, 120));
            g2.fillOval(150, HEIGHT - 50, 80, 60);
        }
    }
    ...
    private void drawParallax(Graphics2D g2) {
        ...
        for (int i = -1; i < 9; i++) {
            ...
            g2.fillPolygon(new int[] { bx, bx + 125, bx + 250 },
                    new int[] { HEIGHT - 20, HEIGHT - 20 - bh, HEIGHT - 20 }, 3);
        }
        ...
        for (int i = -1; i < 7; i++) {
            ...
            g2.fillPolygon(new int[] { bx, midX - 20, midX, midX + 20, bx + 340 },
                    new int[] { HEIGHT - 10, HEIGHT - 10 - bh + 30, HEIGHT - 10 - bh, HEIGHT - 10 - bh + 30,
                            HEIGHT - 10 },
                    5);
        }
        ...
        for (int i = -1; i < 5; i++) {
            ...
            Color wc = night ? new Color(50, 80, 160, 100)
                    : dusk ? new Color(200, 80, 30, 80) : new Color(120, 160, 220, 120);
            g2.setColor(wc);
            g2.fillOval(bx + 100, HEIGHT - 240, 14, 18);
            g2.fillRect(bx + 100, HEIGHT - 230, 14, 10);
            g2.fillRect(bx + 42, HEIGHT - 155, 10, 14);
            g2.fillRect(bx + 166, HEIGHT - 140, 10, 14);
            g2.setColor(ruinCol);
            g2.fillRect(bx + 185, HEIGHT - 80, 50, 60);
            g2.fillRect(bx + 195, HEIGHT - 90, 20, 15);
        }
        // Ground blend
    ...
    private void drawPlatforms(Graphics2D g2) {
        ...
        for (Rectangle p : currentLevel.platforms) {
            ...
            g2.setPaint(new GradientPaint(p.x, p.y, new Color(160, 80, 240, (int) (glow * 200)), p.x, p.y + 7,
                    new Color(80, 40, 160, 0)));
            ...
            for (int rx = p.x + 20; rx < p.x + p.width - 10; rx += 28) {
                ...
                g2.fillPolygon(new int[] { rx + 5, rx + 8, rx + 5, rx + 2 },
                        new int[] { p.y + 5, p.y + 8, p.y + 11, p.y + 8 }, 4);
            }
            if (p.width > 80) {
                drawCrystal(g2, p.x + 12, p.y, glow);
                if (p.width > 140)
                    drawCrystal(g2, p.x + p.width - 20, p.y, glow);
                if (p.width > 220)
                    drawCrystal(g2, p.x + p.width / 2, p.y, glow);
            }
        }
    }
    ...
    private void drawCrystal(Graphics2D g2, int cx, int platTop, float glow) {
        ...
        g2.fillPolygon(new int[] { cx, cx + w / 2, cx + w, cx + w - 1, cx + 1 },
                new int[] { platTop - h, platTop - h - 5, platTop - h, platTop, platTop }, 5);
    ...
    private void drawPortal(Graphics2D g2) {
        ...
        if (active) {
            ...
            int gs = (int) (90 * pulse);
            g2.setColor(new Color(100, 80, 220, 18));
            g2.fillOval(px - gs / 2, py - gs / 4, gs + 60, gs + 60);
            g2.setColor(new Color(140, 100, 255, 30));
            g2.fillOval(px - gs / 3, py - gs / 6, gs + 40, gs + 40);
            g2.setColor(new Color(200, 180, 255, 130));
            g2.fillOval(px - 30, py, 60, 60);
            g2.setColor(new Color(30, 20, 70));
            g2.fillOval(px - 24, py + 4, 48, 48);
            ...
            g2.setColor(new Color(30, 22, 48));
            g2.fillOval(px - 18, py + 10, 36, 36);
            g2.setColor(new Color(60, 40, 90, 100));
            g2.drawOval(px - 18, py + 10, 36, 36);
